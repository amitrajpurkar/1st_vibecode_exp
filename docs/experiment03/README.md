# re-create Weather App using Spec Kit

This project demonstrates the use of Spec Kit to drive the development of a weather data analysis application.
<br/><br/>NOTE: this application is work-in-progress.. code is in feature-branch


## github repo
https://github.com/amitrajpurkar/weather_app_speckit

```bash
git clone git@github.com:amitrajpurkar/weather_app_speckit.git
```


## Features

- Monthly weather trend analysis
- Yearly average temperature and humidity charts
- Detailed monthly trend visualization
- Data aggregation and filtering
- Data validation and error handling
- Spec-driven development approach
- Test-driven development with pytest
- Comprehensive test coverage
- Clear project structure and documentation
- Modular and maintainable codebase
- Well-documented API and data models
- Easy to understand and extend
- Follows Spec Kit methodology
- Adheres to clean architecture principles

## reference..

- check the notes in [SpeckitNotes.pdf](https://github.com/amitrajpurkar/weather_app_speckit/blob/main/src/main/resources/SpeckitNotes.pdf)


## Getting Started

To get started with this project, follow these steps:

1. Clone the repository
2. Install dependencies: `pip install -r requirements.txt`
3. Run the application: `python main.py`